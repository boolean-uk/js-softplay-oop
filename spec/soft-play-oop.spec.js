...
describe("Soft Play OOP", () => {
  ...
  // EXTENSION //
  it("cannot exceed max capacity", function () {
    let softPlay = new SoftPlay(1);
    expect(softPlay.enter(2, 1)).toBeFalse();
    expect(softPlay.occupancy()).toEqual({ adults: 0, children: 0 });

    expect(softPlay.enter(1, 0)).toBeTrue();
    expect(softPlay.occupancy()).toEqual({ adults: 1, children: 0 });
  });
